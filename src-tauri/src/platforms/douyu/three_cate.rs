...
fn transform_three_cate_to_common(raw_items: Vec<DouyuThreeCateItemRaw>, parent_cate_id: &str) -> Vec<CommonPlatformCategoryRust> {
    raw_items.into_iter().map(|raw_item| CommonPlatformCategoryRust {
        id: raw_item.id,
        name: raw_item.name,
...
pub async fn fetch_three_cate(tag_id: i32) -> Result<Vec<CommonPlatformCategoryRust>, String> {
    ...
    let url = format!("https://capi.douyucdn.cn/api/v1/getThreeCate?tag_id={}&client_sys=android", tag_id_str);
    info!("[API Command] fetch_three_cate called for tag_id: {}", tag_id_str);

    match reqwest::get(&url).await {
        Ok(response) => {
            if response.status().is_success() {
                let body_text = response.text().await.map_err(|e| format!("Failed to read response text: {}", e))?;
                        ...
                        if parsed_response.error == 0 {
                            ...
                            if let Some(items_list) = parsed_response.data {
                                if !items_list.is_empty() {
                                    let common_data = transform_three_cate_to_common(items_list, &tag_id_str);
                                    Ok(common_data)
                                } else {
                            ...
                            Err(format!("ThreeCate API error for tag_id {}. Code: {}, Msg: {:?}", tag_id_str, parsed_response.error, parsed_response.msg))
                        }
                    }
                    Err(e) => Err(format!("Failed to parse three_cate JSON for tag_id {}: {}, Body: {}", tag_id_str, e, body_text))
                }
            } else {
                let status = response.status();
                let error_text = response.text().await.unwrap_or_else(|_| "Unknown error from API".to_string());
                error!("fetch_three_cate API request failed for tag_id {} with status {}: {}", tag_id_str, status, error_text);
                Err(format!("API request failed for tag_id {} with status {}: {}", tag_id_str, status, error_text))
            }
        }
        Err(e) => {
            error!("fetch_three_cate request failed for tag_id {}: {}", tag_id_str, e);
