...
pub async fn start_douyin_danmu_listener(
    ...
    println!("[Douyin Danmaku] Received request for room_id_or_url: {}", room_id_or_url);
    ...
    if room_id_or_url == "stop_listening" {
        println!("[Douyin Danmaku] Received stop_listening signal. Listener will not be restarted.");
        return Ok(());
    }
    ...
    tokio::spawn(async move {
        println!("[Douyin Danmaku] Spawning listener for room: {}", room_id_str_clone);
        ...
        if let Err(e) = task_result {
            eprintln!("[Douyin Danmaku] Listener task for room {} critically failed: {}", room_id_str_clone, e);
            let error_payload = crate::platforms::douyin::danmu::message_parsers::DanmakuFrontendPayload {
            ...
            if let Err(emit_err) = app_handle.emit("danmaku-message", error_payload) {
                 eprintln!("[Douyin Danmaku] Failed to emit error event to frontend: {}", emit_err);
            }
        } else {
            println!("[Douyin Danmaku] Listener task for room {} completed.", room_id_str_clone);
        }
    });
    Ok(())
} 