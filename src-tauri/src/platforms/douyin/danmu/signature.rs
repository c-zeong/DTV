...
pub async fn generate_signature(wss_url: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    for key_str in params_to_sign_keys {
        let value = query_params_map.get(key_str).map(|s| s.as_str()).unwrap_or("");
    ...
    "#;
    runtime.execute_script("[bootstrap]", FastString::from_static(bootstrap_script)).map_err(|e| {
        eprintln!("Error during deno_core bootstrap script: {}", e);
        e
    })?;

    // Use the embedded sign.js content
    runtime.execute_script("./sign.js", FastString::from_static(SIGN_JS_CONTENT)).map_err(|e| {
    ...
    let result = runtime.execute_script("[call_get_sign]", fast_call_script).map_err(|e| {
    ...
    if local_value.is_string() {
        ...
        Err(Box::from("get_sign did not return a string value from deno_core") as Box<dyn std::error::Error + Send + Sync>)
    }
}
