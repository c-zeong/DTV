...
pub fn parse_chat_message(payload: &[u8], current_room_id: &str) -> Result<Option<DanmakuFrontendPayload>, Box<dyn std::error::Error + Send + Sync>> {
    match ChatMessage::decode(payload) {
        Ok(chat_msg) => {
            if let Some(user) = chat_msg.user {
                ...
                let fans_club_level = user.fans_club.as_ref()
                ...
                println!("    【聊天msg】Content: {} (no user info)", chat_msg.content);
                Ok(Some(DanmakuFrontendPayload {
                    room_id: current_room_id.to_string(), // Populate room_id
...
pub fn parse_member_message(payload: &[u8], _current_room_id: &str) -> Result<Option<DanmakuFrontendPayload>, Box<dyn std::error::Error + Send + Sync>> {
...
#[allow(dead_code)] // ADDED to suppress warning
pub fn parse_like_message(payload: &[u8], _current_room_id: &str) -> Result<Option<DanmakuFrontendPayload>, Box<dyn std::error::Error + Send + Sync>> {
