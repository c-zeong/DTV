...
pub async fn fetch_live_list_for_cate3(cate3_id: String, page: u32, limit: u32) -> FrontendLiveListResponse {
    ...
    println!("[Backend fetch_live_list_for_cate3] Raw Douyu Response: {}", text);

    match serde_json::from_str::<DouyuV1ApiResponse>(&text) {
        Ok(douyu_response) => {
            if douyu_response.code == 0 {
                if let Some(douyu_data) = douyu_response.data {
                    let streamers_transformed: Vec<FrontendStreamer> = douyu_data.rl.into_iter().map(|s| {
                        let hn_formatted = if s.ol >= 10000 {
                            format!("{:.1}万", s.ol as f64 / 10000.0)
                        } else {
                            s.ol.to_string()
                        };
                        FrontendStreamer {
                            ...
                            room_src: s.rs16,
                            hn: hn_formatted,
                            is_live: Some(s.stream_type.map_or(true, |t| t == 1)),
                        }
                    }).collect();

                    // Estimate total for this API as it often doesn't provide it.
                    // Assume a page size (e.g., 30 items per page is common for this Douyu API).
                    let assumed_api_page_size = 30_u32; 
                    let num_fetched = streamers_transformed.len() as u32;
                    let estimated_total = if num_fetched < assumed_api_page_size {
                        // This is likely the last page
                        (current_page.saturating_sub(1)) * assumed_api_page_size + num_fetched
                    } else {
                        // Assume at least one more page exists if we fetched a full page
                        current_page * assumed_api_page_size + 1 
                    };
        ...
        Err(e) => {
            eprintln!("[Backend fetch_live_list_for_cate3] Error parsing Douyu JSON: {}. Raw: {}", e, text);
            FrontendLiveListResponse {
                error: -2,
                msg: Some(format!("Failed to parse Douyu API response: {}", e)),
                data: None,
