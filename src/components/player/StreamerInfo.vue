...
const fetchRoomInfo = async () => {
  if (!props.roomId) {
    ...
    roomTitle.value = ''; nickname.value = 'N/A'; isLive.value = false; isReplay.value = false;
    avatarUrl.value = ''; categoryName.value = ''; viewerCount.value = 0; showAvatarText.value = true;
    return
  }

  console.log('StreamerInfo: Fetching room info for ' + props.roomId);
  roomTitle.value = '加载中...'; nickname.value = '加载中...'; isLive.value = false; isReplay.value = false;
  avatarUrl.value = ''; categoryName.value = ''; viewerCount.value = 0; showAvatarText.value = false;

  try {
    const data = await invoke<RoomInfo>('fetch_room_info', { roomId: props.roomId })
    console.log('StreamerInfo fetched data:', data);
    
    if (!data || !data.room) {
      console.error('StreamerInfo: Invalid data received from fetch_room_info', data);
      roomTitle.value = '信息获取失败'; nickname.value = '主播信息错误';
      return
    }

    roomTitle.value = data.room.room_name || '未知房间名'
    ...
    categoryName.value = data.room.cate_name || '游戏分区'
    viewerCount.value = Number(data.room.online_num) || 0
    ...
    console.error('StreamerInfo: 获取房间信息失败:', error)
    roomTitle.value = '信息加载失败'; nickname.value = '主播信息错误';
  }
}
...
watch(() => props.roomId, (newId, oldId) => {
  console.log('StreamerInfo: roomId changed from ' + oldId + ' to ' + newId);
  if (newId) {
    fetchRoomInfo()
  } else {
    roomTitle.value = ''; nickname.value = 'N/A'; isLive.value = false; isReplay.value = false;
