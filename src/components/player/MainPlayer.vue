...
import StreamerInfo from '../player/StreamerInfo.vue';
import DanmuList from '../player/DanmuList.vue';
...
const initPlayer = async () => {
  ...
  try {
    ...
    console.log('[MainPlayer] Fetching stream URL for room:', props.roomId);
    const streamUrl = await invoke<string>('get_stream_url_cmd', { roomId: props.roomId });
    console.log('[MainPlayer] Stream URL received:', streamUrl);

    if (!streamUrl) {
      console.error('[MainPlayer] Received empty stream URL. Aborting player init.');
      return;
    }
      ...
      customType: {
        flv: function(video: HTMLVideoElement, url: string) {
          import('mpegts.js').then(mpegts => {
            ...
            video.play().catch(e => console.error('[MainPlayer] Auto-play error:', e));
            ...
            player.on('statistics_info', (statInfo) => {
              console.log('[mpegts] Statistics:', statInfo);
            });
          }).catch(err => console.error('[MainPlayer] Error loading mpegts.js:', err));
        }
      },
      plugins: [
