<template>
  <div class="danmu-list-wrapper">
    ...
    <div class="danmu-messages-area" ref="danmakuListEl" @scroll="handleScroll">
      ...
      <div v-for="(danmaku, index) in danmakuList" :key="index" class="danmu-item">
        <div class="danmu-meta-line">
          ...
          <span class="danmu-user" :style="{ color: userColor(danmaku.nickname) }">
...
interface DanmakuMessage {
  type?: string; // To handle different message types from Douyu, e.g., 'chatmsg'
  uid?: string;
  nickname: string;
  level: string;
  content: string;
  badgeName?: string;
  badgeLevel?: string;
  color?: string; // Douyu specific color for danmu
  // other potential fields: ic, bnn, bl, Brid, hc, ol, cst, gfid, anbg, fl, dms // etc.
}
...
const setupDanmakuListener = async (roomIdToListenFor: string) => {
  ...
  try {
    await invoke('start_danmaku_listener', { roomId: currentRoomId });
    
    unlistenDanmaku = await listen<DanmakuMessage>('danmaku', (event) => {
      // console.log('[DanmuList] Received global danmaku event:', JSON.stringify(event.payload));
      
      if (event.payload.type === 'chatmsg' || !event.payload.type) {
        ...
        scrollToBottom();
      } else if (event.payload.type === 'uenter') {
        // console.log('[DanmuList] User entered:', event.payload.nickname);
      }
    });
